...
class Agent():
    ...
    #############################################################################################
    def evaluate_frontiers(self, frontier_idxs, krm):
        ''' 
        ...
        for frontier_idx in frontier_idxs:
            candidate_path = nx.shortest_path(
                krm.KRM, source=self.at_wp, target=frontier_idx)
    ...
    def find_path_to_selected_frontier(self, target_frontier, krm):
        '''
        ...
        path = nx.shortest_path(
            krm.KRM, source=self.at_wp, target=target_frontier)
        return path

    def sample_waypoint(self, krm):
        '''
        ...
        '''
        wp_at_previous_pos = krm.get_node_by_pos(self.previous_pos)
        krm.add_waypoint(self.pos, wp_at_previous_pos)
        self.at_wp = krm.get_node_by_pos(self.pos)
