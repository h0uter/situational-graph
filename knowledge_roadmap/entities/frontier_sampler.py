...
class FrontierSampler():
    ...
    def plot_collision(self, data, r, c, to, at, local_grid_adapter):
        plt.figure(9)
        x_meter, y_meter = local_grid_adapter.pix_idx2world_coord(data, c, r)

        plt.cla()
        plt.ion()
        plt.imshow(data, origin='lower')
        *at_meters, = local_grid_adapter.pix_idx2world_coord(data, at[1], at[0])
        *to_meters, = local_grid_adapter.pix_idx2world_coord(data, to[1], to[0])
        print(at_meters)
    ...
    def collision_check(self, data, at, to, local_grid_adapter):
    ...
    def sample_frontiers(self, local_grid, local_grid_adapter, radius=90, num_frontiers_to_sample=5) -> list:
        ...
        while len(candidate_frontiers) < num_frontiers_to_sample:
            ...
            x_sample, y_sample = local_grid_adapter.pix_idx2world_coord(local_grid, y_sample, x_sample)
