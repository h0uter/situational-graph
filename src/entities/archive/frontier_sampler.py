...
# TODO: merge this class into local grid
class FrontierSampler():
    ...
    def get_cells_under_line(self, point_a, point_b) -> tuple:
        ...
        rr, cc = draw.line(int(point_a[0]), int(
            point_a[1]), int(point_b[0]), int(point_b[1]))
    ...
    def plot_collision_cell_map(self, data: list, r: int, c: int, to: tuple, at: tuple) -> None:
        ...
        # FIXME: this can be cleaned up, after that local_pix_idx2world_coord function turned out to be useless
        x_meter, y_meter = c, r
        ...
        *to_meters, = to[1], to[0]
        print(at_meters)
        plt.plot([at_meters[0], to_meters[0]], [
                 at_meters[1], to_meters[1]], color='green')
