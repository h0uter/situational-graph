...
def main(argv):
    ...
    lcfg = get_login_config()
    # robot = sdk.create_robot(options.hostname)
    robot = sdk.create_robot(lcfg.wifi_hostname)

    # robot.authenticate(options.username, options.password)
    robot.authenticate(lcfg.username, lcfg.password)

    # Create estop client for the robot
    estop_client = robot.ensure_client(EstopClient.default_service_name)

    # exit(build_and_run_app(options.hostname, estop_client, options))
    exit(build_and_run_app(lcfg.wifi_hostname, estop_client))
